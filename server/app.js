...
// Setup the LTI provider (following the newer ltijs pattern)
...
// 3) Main async function to start the application
const startApp = async () => {
  try {
    ...
    await require('./platform-registry')(Provider)

    // harden Express
    Provider.app.set('trust proxy', 1)
    
    // Setup access logging
    Provider.app.use(morgan('combined', { stream: accessLogStream }))
    Provider.app.use(morgan('combined')) // Also log to console
    
    Provider.app.use(helmet({ 
      crossOriginEmbedderPolicy: false,
      contentSecurityPolicy: false // Disable CSP to allow your game's inline scripts
    }))
    Provider.app.use(rateLimit({ windowMs: 60_000, max: 120 }))
    Provider.app.use(express.json())

    // allow only LMS to frame us
    const frameAncestors = (process.env.FRAME_ANCESTORS || '').trim()
    Provider.app.use((req, res, next) => {
      if (frameAncestors) {
    ...
    Provider.app.use('/static', express.static(path.join(__dirname, '../public')))

    // Your existing grade endpoint adapted to the new structure
    Provider.app.post('/api/grade', async (req, res) => {
      try {
        ...
        const items = await Provider.Grade.getLineItems(token, { resourceLinkId, tag: 'visual-search' })
        const lineItem =
          items?.[0] ||
          (await Provider.Grade.createLineItem(token, {
        ...
        await Provider.Grade.submitScore(token, lineItemId, {
          userId: token.user,
          scoreGiven: score,
          scoreMaximum: max,
    ...
    Provider.app.post('/grade', async (req, res) => {
      try {
        ...
        if (!lineItemId) {
          const response = await Provider.Grade.getLineItems(token, { resourceLinkId: true })
          ...
          if (lineItems.length === 0) {
            ...
            const lineItem = await Provider.Grade.createLineItem(token, newLineItem)
        ...
        const responseGrade = await Provider.Grade.submitScore(token, lineItemId, gradeObj)
